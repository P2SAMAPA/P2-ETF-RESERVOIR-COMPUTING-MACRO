...
st.set_page_config(page_title="Reservoir Computing with Macro‑Driven Spectral Radius", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Reservoir Computing with Macro‑Driven Spectral Radius</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Echo State Network | Spectral radius adapted to macro (VIX) | Readout weight as score</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 ESN")
...
st.sidebar.markdown(f"**Reservoir size:** {config.RESERVOIR_SIZE} | **Base ρ:** {config.SPECTRAL_RADIUS_BASE}")
st.sidebar.markdown(f"**ρ range:** {config.SPECTRAL_RADIUS_RANGE} | **Connectivity:** {config.CONNECTIVITY}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'reservoir_computing_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>ESN score: {etf['esn_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized ESN Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized ESN Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Echo State Network Prediction (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Reservoir computing** (Echo State Network) uses a fixed random recurrent reservoir with a trained linear readout.
        - The **spectral radius** of the reservoir matrix is adapted to macro (e.g., VIX): high VIX → smaller radius → more stable dynamics.
        - The score is the predicted next‑day return from the readout.
        - High score → expected upward move.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the ESN scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Reservoir Computing | Echo State Network with macro‑driven spectral radius")
