...
from reservoir_computing import reservoir_computing_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    top_etfs = [{"ticker": t, "esn_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "reservoir_size": config.RESERVOIR_SIZE,
        "spectral_radius_base": config.SPECTRAL_RADIUS_BASE,
        "spectral_radius_range": config.SPECTRAL_RADIUS_RANGE,
        "connectivity": config.CONNECTIVITY,
        "ridge_alpha": config.RIDGE_ALPHA,
        "universes": {}
    }
    ...
    out_file = f"output/reservoir_computing_{timestamp}.json"
